...
import { HttpClient, HttpHeaders, HttpErrorResponse } from '@angular/common/http'; // Import HttpClient and HttpHeaders for HTTP requests
...
export class AuthService {
  ...
  login(email: string, password: string) {
    ...
    return this.http.post<AuthResponseData>(this.baseURL + 'api/auth/signin', null, httpOptions).pipe(
      catchError(err => {
  ...
  register(user: RegisterUser) {
    return this.http.post<RegisterUser>(this.baseURL + 'api/auth/signup', user).pipe(
      catchError(err => {
        let errorMessage = 'An unknown error occurred!';
...
/*  logout() {
    this.http.request('post', this.baseURL + 'api/auth/signout', {
      withCredentials: true // Include credentials (cookies) in the request
    }).subscribe({
      next: () => {
    ...
    logout(): Observable<void> {
      return this.http.request<void>('post', this.baseURL + 'api/auth/signout', {
        withCredentials: true // Inclure les credentials (cookies) dans la requête
      }).pipe(
        tap(() => {
          this.storageService.clean(); // Nettoyer le stockage local
          this.AuthenticatedUser$.next(null); // Réinitialiser l'utilisateur authentifié
        }),
